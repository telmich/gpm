...
int M_imps2(Gpm_Event * state, unsigned char *data)
{
   ...
   if(wheel > 0) {
      ...
      switch (wheel) {
            /*
             * rodney 13/mar/2008 The use of GPM_B_UP / GPM_B_DOWN is very
             * unclear; only mouse type ms3 uses these For this mouse, we only
             * support the relative movement i.e. no button is set (same as
             * mouse movement), wdy changes +/- according to wheel movement (+
             * for rolling away from user) wdx (horizontal scroll) is for a
             * second wheel. They do exist! 
             */
         case 0x0f:
            state->wdy = +1;
            break;
