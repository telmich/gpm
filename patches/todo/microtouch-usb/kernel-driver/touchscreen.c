...
struct tscrn_usb_data {
   ...
   unsigned char data[TSCRN_USB_RAPORT_SIZE_DATA];      // Data buffer X-Y, and 
                                                        // button 
   ...
   unsigned char *pToRead, *pToWrite;   // pointers to toRead cell of buffer
                                        // and to toWrite one 
   char intr_ep;                // Endpoint assignments 
   struct input_dev input_dev;  // to work as an input device driver
};

static struct tscrn_usb_data *p_tscrn_table[TSCRN_MAX_MNR] =
   { NULL, /* ... */  };
...
static void irq_touchscreen(struct urb *urb)
{
    ...
    */
   struct tscrn_usb_data *tscrn = urb->context;
   struct tscrn_usb_data_report *data =
      (struct tscrn_usb_data_report *) tscrn->data;
   ...
   // warn("warn:Lost data new loop %d previous loop
   // %d",(int)GET_LOOP(data),(int)tscrn->nLoopCounter);
   ...
   // dbg("irq_touchscreen(): GET_Data
   // 0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x status=%d",
   // (int)data[0],(int)data[1],(int)data[2],(int)data[3],(int)data[4],(int)data[5],(int)data[6],(int)data[7],(int)data[8],(int)data[9],(int)data[10], 
   // tscrn->ctrlout.status);
                                ...
                                size_t count, loff_t * ppos)
{
   ...
   this_read =
      (count >=
       TSCRN_USB_RAPORT_SIZE_DATA) ? TSCRN_USB_RAPORT_SIZE_DATA : count;

   partial = this_read;
   // dbg("read stats(%d): result:%d this_read:%d partial:%d", minor, result,
   // this_read, partial);
...
*/
static unsigned int poll_touchscreen(struct file *file,
                                     struct poll_table_struct *_wait)
{
   struct usb_device *dev;
   struct tscrn_usb_data *tscrn;
   struct inode *inode;
   ...
   switch (cmd) {
      case TSCRN_USER_REQUEST:
      {
         ...
         FILL_CONTROL_URB(&tscrn->ctrlout, dev,
                          ...
                          (unsigned char *) tscrn->setup_packet, NULL, 0,
                          ctrl_touchscreen, tscrn);

         if((nRet = usb_submit_urb(&tscrn->ctrlout))) {
            err("ioctl_touchscreen(%d): errror=%d status=%d.", minor, nRet,
                tscrn->ctrlout.status);
            return nRet;
         }
         dbg("ioctl_touchscreen(%d): cmd status=%d nRet=%d", minor,
             tscrn->ctrlout.status, nRet);
         interruptible_sleep_on(&tscrn->wait);
         dbg("ioctl_touchscreen(%d): cmd status=%d", minor,
             tscrn->ctrlout.status);
      ...
      {
         tscrn->setup_packet->requesttype =
         ...
         dbg("ioctl_touchscreen(%d): status=%d nRet=%d", minor,
             tscrn->ctrlout.status, nRet);
         interruptible_sleep_on(&tscrn->wait);
      ...
      case TSCRN_CONTROLLER_STATUS:
      {
         ...
         FILL_CONTROL_URB(&tscrn->ctrlin, dev,
                          ...
                          (unsigned char *) tscrn->setup_packet, tscrn->ibuf,
                          TSCRN_USB_RAPORT_SIZE_STATUS, ctrl_touchscreen,
                          tscrn);
         ...
         if(tscrn->ctrlin.status == 0)
            if(copy_to_user
               ((void *) arg, tscrn->ibuf, TSCRN_USB_RAPORT_SIZE_STATUS))
               return -EFAULT;
      ...
      case TSCRN_CONTROLLER_ID:
      {
         ...
         FILL_CONTROL_URB(&tscrn->ctrlin, dev,
                          usb_rcvctrlpipe(dev, 0x80),
                          (unsigned char *) tscrn->setup_packet, tscrn->ibuf,
                          TSCRN_USB_RAPORT_SIZE_ID, ctrl_touchscreen, tscrn);
         ...
         if(tscrn->ctrlin.status == 0)
            if(copy_to_user
               ((void *) arg, tscrn->ibuf, TSCRN_USB_RAPORT_SIZE_ID))
               return -EFAULT;
      ...
      case TSCRN_CALIBRATION:
      {
         ...
         if(copy_from_user(&type, (void *) arg, sizeof(type))) {
            err("ioctl_touchscreen(%d): copy from user error arg=%d.", minor,
                *((int *) arg));
            return -EFAULT;
         };
         ...
         switch (type) {
            ...
            case TSCRN_CORNER_CALIBRATION_TYPE:
               break;
            default:
               err("ioctl_touchscreen(%d): unknown calibration type %d.", minor,
                   type);
               return -EFAULT;
...
static void *probe_touchscreen(struct usb_device *dev, unsigned int ifnum)
{
   ...
   if(vendor != -1 && product != -1) {
      info
         ("probe_touchscreen: User specified USB touchscreen -- Vendor:Product - %x:%x",
          vendor, product);
   }
   ...
   dbg("probe_touchscreen: Number of Endpoints:%d",
   ...
   if(IS_EP_INTR(endpoint[ep_cnt])) {
      ...
      return NULL;              /* Shouldn't ever get here unless we have
                                 * something weird */
   }
   ...
   if(!(tscrn->obuf = (char *) kmalloc(OBUF_SIZE, GFP_KERNEL))) {
      err("probe_touchscreen(%d): Not enough memory for the output buffer.",
          minor);
      kfree(tscrn);
      return NULL;
   }
   ...
   if(!(tscrn->ibuf = (char *) kmalloc(IBUF_SIZE, GFP_KERNEL))) {
      err("probe_touchscreen(%d): Not enough memory for the input buffer.",
          minor);
      kfree(tscrn->obuf);
      kfree(tscrn);
      return NULL;
   ...
   if(!
      (tscrn->setup_packet =
       (void *) kmalloc(sizeof(*tscrn->setup_packet), GFP_KERNEL))) {
   ...
   dbg("probe_touchscreen(%d): setup_packet address:%p, size=%d size2=%d",
       minor, tscrn->setup_packet, sizeof(*tscrn->setup_packet),
       sizeof(devrequest));
   ...
   tscrn->input_dev.keybit[LONG(BTN_MOUSE)] = BIT(BTN_LEFT);    // |
                                                                // BIT(BTN_RIGHT) 
                                                                // |
                                                                // BIT(BTN_MIDDLE);
   ...
   if(!(nRet = usb_interface_claimed(&dev->actconfig[0].interface[ifnum]))) {
      ...
      usb_driver_claim_interface(&touchscreen_driver,
                                 &dev->actconfig[0].interface[ifnum], tscrn);
   } else
      dbg("probe_touchscreen(%d): error interface_claimed=%d", minor, nRet);
   dbg("probe_touchscreen(%d): Configuring CTRL handler for intr EP:%d", minor,
       have_intr);
   /*
    * sends reset command 
    */
   ...
   FILL_CONTROL_URB(&tscrn->ctrlin, dev,
                    usb_rcvctrlpipe(dev, 0x80),
                    (unsigned char *) tscrn->setup_packet, tscrn->ibuf,
                    sizeof(tscrn->ibuf), ctrl_touchscreen, tscrn);
   ...
   if(tscrn->ctrlin.status == (-32)) {
      dbg("probe_touchscreen(%d): get error -32 status=%d - reset pipe", minor,
          tscrn->ctrlin.status);
      nRet = usb_clear_halt(dev, 0x00);
      dbg("probe_touchscreen(%d): get error -32 status=%d - reset pipe nRet=%d",
          minor, tscrn->ctrlin.status, nRet);
   }
   // 
   // receive answer data
   tscrn->setup_packet->requesttype =
   ...
   if((nRet = usb_submit_urb(&tscrn->ctrlin))) {
      err("probe_touchscreen(%d): GET_STATUS:Get smth error=%d status=%d .",
          minor, nRet, tscrn->ctrlin.status);
      kfree(tscrn->ibuf);
      kfree(tscrn->obuf);
      kfree(tscrn->setup_packet);
      kfree(tscrn);
   ...
   dbg("probe_touchscreen(%d): GET_STATUS: status=%d nRet=%d", minor,
       tscrn->ctrlin.status, nRet);
   interruptible_sleep_on(&tscrn->wait);
   ...
   if((nRet = usb_submit_urb(&tscrn->ctrlout))) {
      err("probe_touchscreen(%d): GET_ID:error=%d status=%d .", minor, nRet,
          tscrn->ctrlout.status);
      kfree(tscrn->ibuf);
      kfree(tscrn->obuf);
      kfree(tscrn->setup_packet);
    ...
    */
   if(have_intr) {
      dbg("probe_touchscreen(%d): Configuring IRQ handler for intr EP:%d",
          minor, have_intr);
      FILL_INT_URB(&tscrn->irq, dev, usb_rcvintpipe(dev, 0x81), // endpoint[(int 
                                                                // )(have_intr-1)].bEndpointAddress),
                   tscrn->data, TSCRN_USB_RAPORT_SIZE_DATA, irq_touchscreen,
                   tscrn, endpoint[(int) (have_intr - 1)].bInterval);
      // 2);
      ...
      // the status will be -115 -> thist means EINPROGRESS, but it should be
      // that.
      dbg("probe_touchscreen(%d): IRQ done interval=%d status=%d", minor,
          endpoint[(int) (have_intr - 1)].bInterval, tscrn->irq.status);
   }
...
static void disconnect_touchscreen(struct usb_device *dev, void *ptr)
{
   ...
   usb_driver_release_interface(&touchscreen_driver,
                                &tscrn->dev->actconfig->interface[tscrn->
                                                                  ifnum]);
