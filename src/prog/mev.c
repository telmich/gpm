...
int user_handler(Gpm_Event * event, void *data)
{
   ...
   printf("mouse: event 0x%02X, at %2i,%2i (delta %2i,%2i), "
          ...
          event->type,
          event->x, event->y,
          event->dx, event->dy, event->buttons, event->modifiers);
...
int emacs_handler(Gpm_Event * event, void *data)
{
    ...
    * alphabetical order, so I'll use a lookup table instead of a loop; it is
    * faster anyways 
    */
   /*
   ...
   /*
    * itz Mon Mar 23 08:23:14 PST 1998 what emacs calls a `drag' event is our
    * `up' event with coordinates different from the `down' event.  What gpm
   ...
   /*
    * itz Mon Mar 23 08:27:53 PST 1998 this flag is needed because even if the
    * final coordinates of a drag are identical to the initial ones, it is
    * still a drag if there was any movement in between.  Sigh. 
    */
   ...
   /*
    * itz Sun Mar 22 19:09:04 PST 1998 Emacs doesn't understand modifiers on
    * motion events. 
    */
   ...
   if((event->type & GPM_UP) && dragFlag) {
      printf("(%s ((%i . %i) %ld) %c ((%i . %i) %ld))\n",
             buffer,
             event->x, event->y, timestamp, count, dragX, dragY, dragTime);
...
void getmask(char *arg, int which, int *where)
{
   ...
   while(1) {
      ...
      *cur = 0;
      ...
      if(!n->name)
         fprintf(stderr, "%s: Incorrect flag \"%s\"\n", prgname, arg);
      if(last)
         break;
...
int cmdline(int argc, char **argv, const char *options)
{
   ...
   while((opt = getopt(argc, argv, options)) != -1) {
      switch (opt) {
            /*
             * itz Tue Mar 24 17:11:52 PST 1998 i hate options that do too
             * much.  Made them orthogonal. 
             */

         case 'C':
            sscanf(optarg, "%x", &opt_vc);
            break;
         case 'd':
            getmask(optarg, PARSE_EVENTS, &opt_default);
            break;
         case 'e':
            getmask(optarg, PARSE_EVENTS, &opt_mask);
            break;
         case 'E':
            opt_emacs = 1;
            break;
         ...
         case 'f':
            ...
            break;
         case 'm':
            getmask(optarg, PARSE_MODIFIERS, &opt_minMod);
            break;
         case 'M':
            getmask(optarg, PARSE_MODIFIERS, &opt_maxMod);
            break;
         case 'p':
            opt_pointer = 1;
            break;
...
{                               /* returns 0 on success and !=0 on error */
   ...
   if(!strcmp(argv[0], "pop")) {
      return (Gpm_Close() == 0 ? 1 : 0);        /* a different convention on
                                                 * ret values */
   }
   /*
    * if
...
int main(int argc, char **argv)
{
   ...
   if(Gpm_Open(&conn, opt_vc) == -1) {
      ...
      gpm_report(GPM_PR_OOPS,
                 "%s: use rmev to see gpm events in xterm or rxvt\n", prgname);
   }
