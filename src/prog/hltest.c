...
int handler(Gpm_Event * ePtr, void *clientdata)
{
   ...
   if(ePtr->type & GPM_DRAG) {
      ...
      delta = 0;
      if(ePtr->dx < 0)
         delta = max(ePtr->dx, LEFTWID - info->roi->xMin);
      else if(ePtr->dx > 0)
         delta = min(ePtr->dx, wid - 1 - info->roi->xMax);
      if(delta) {
         info->roi->xMin += delta;
         info->roi->xMax += delta;
      }
      ...
      delta = 0;
      if(ePtr->dy < 0)
         delta = max(ePtr->dy, 0 - info->roi->yMin);
      else if(ePtr->dy > 0)
         delta = min(ePtr->dy, hei - 1 - info->roi->yMax);
      if(delta) {
         info->roi->yMin += delta;
         info->roi->yMax += delta;
      }
 ...
 */
   ...
   if(!(ePtr->type & GPM_SINGLE))
      return 0;
   if(ePtr->x < 0 || ePtr->y < 0)
      return 0;
   if(ePtr->x > info->wid - 1 || ePtr->y > info->hei - 1)
      return 0;
...
int main(int argc, char **argv)
{
   ...
   hei = win.ws_row;
   vcsize = hei * wid * 2 + 4;
   dumpbuf = malloc(vcsize);
   dumpbuf_clean = malloc(vcsize);
    ...
    */
   left_cldata.color = 0x07;
   left_cldata.number = 0;
   left_cldata.hei = hei;
   left_cldata.wid = LEFTWID;
