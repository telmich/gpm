...
void startup(int argc, char **argv)
{
   ...
   static struct {
      ...
      "123", "01234567"}, {
      "132", "02134657"}, {
      "213", "01452367"},       /* warning: these must be readable as
                                 * integers... */
      {
      "231", "02461357"}, {
      "312", "04152637"}, {
   ...
   if(geteuid() != 0)
      gpm_report(GPM_PR_OOPS, GPM_MESS_ROOT);   /* root or exit */
   ...
   for(i = 1; i <= 1 + opt_double; i++) {
      ...
      if((which_mouse->opt_accel) < 1)
         exit(usage("acceleration"));
      if((which_mouse->opt_delta) < 2)
         exit(usage("delta"));
      if(strlen((which_mouse->opt_sequence)) != 3
         || atoi((which_mouse->opt_sequence)) < 100)
         exit(usage("sequence"));
      if((which_mouse->opt_glidepoint_tap) > 3)
         exit(usage("glidepoint tap button"));
      if((which_mouse->opt_glidepoint_tap))
         (which_mouse->opt_glidepoint_tap) =
            GPM_B_LEFT >> ((which_mouse->opt_glidepoint_tap) - 1);

      /*
       * choose the sequence 
       */
      for(opt = 0;
          seq[opt].in && strcmp(seq[opt].in, (which_mouse->opt_sequence));
          opt++) ;
      if(!seq[opt].in)
         exit(usage("button sequence"));
      (which_mouse->opt_sequence) = strdup(seq[opt].out);       /* I can
                                                                 * rewrite on
                                                                 * it */
      ...
      if(which_mouse->opt_calib != NULL) {
         ...
         if(calib_file) {
            if(fscanf(calib_file, "%i %i %i %i %i %i %i %i",
                      &which_mouse->opt_dminx, &which_mouse->opt_dmaxx,
                      &which_mouse->opt_dminy, &which_mouse->opt_dmaxy,
                      &which_mouse->opt_ominx, &which_mouse->opt_omaxx,
                      &which_mouse->opt_ominy, &which_mouse->opt_omaxy) != 8)
