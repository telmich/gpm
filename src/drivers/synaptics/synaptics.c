...
static int pressure_speed_enabled = 1;  /* does pressure control C */
                                                  ...
                                                  * speed in non edges 
                                                  */
static int tap_hold_edge_motion_enabled = 1;    /* Enable edge motion while
                                                 * holding a tap C */
...
static int multi_finger_tap_enabled = 1;        /* No of fingers decides which
                                                 * button is pressed C */
static int multi_finger_stop_enabled = 1;       /* less sensitive mouse with
                                                 * multi finger C */
static int multi_finger_stop_delay = 8; /* how long to stop after a multifinger 
                                         * detection. C */
static int fake_finger_layer_enabled = 1;       /* add an extra software layer
                                                 * to detect multi fingers C */

/* mixed configurations */
static int touchpad_enabled = 1;        /* Disable the touch pad, see corner
                                         * action turn_on_off. C */
static int stick_enabled = 1;   /* Some machines have a touchpad and a stick on
                                 * same device port. The stick will be ignored
                                 * in absolute mode, this option try to
                                 * recognize it's packets.  C */
static int stick_pressure_enabled = 1;  /* A (hard) press on the stick is
                                         * reported as left click C */
static int four_way_button_enabled = 1; /* Round button giving 4 choices on
                                         * some touchpads C */
static int four_way_button_is_mouse = 1;        /* Is the button: 4 buttons or
...
static param_data_type param_data[] = {
   ...
   {"tap_hold_edge_motion_enabled", Flag_Param,
    {&tap_hold_edge_motion_enabled}},
...
#define PS2_NO_SLEEP       0x00 /* Bit (only button press activates touchpad
                                 * again) */
...
#define STICK_DISABLE      0x02 /* Bit 1 for ps2 protocol this disables any
                                 * stick attached */
...
static int fake_time_to_forget_tap = 0; /* not to lose user defined actions in
                                         * the hold periode. (action like:
                                         * Multifingers, non-repeating actions
                                         * etc. */
                                ...
                                int edges, Gpm_Event * state)
{
   ...
   gpm_report(GPM_PR_DEBUG,
              ...
              (multi_finger_pressure > 4500
               && multi_finger_xy > 50000 ? 'f' : ' '));
...
static void syn_dump_info(int stick)
{
   ...
   gpm_report(GPM_PR_INFO, "Synaptic %s Device:",
              (stick ? "Stick" : "Touchpad"));

   gpm_report(GPM_PR_INFO,
              "Synaptics Ident:  model_code=%d   Firmware version %d.%d",
              ident[stick].info_model_code, ident[stick].info_major,
              ident[stick].info_minor);
   gpm_report(GPM_PR_INFO, "Synaptics model:");
   gpm_report(GPM_PR_INFO, "   rot180:      %s",
              model[stick].info_rot180 ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   portrait:    %s",
              model[stick].info_portrait ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   sensor:      %d", model[stick].info_sensor);
   gpm_report(GPM_PR_INFO, "                %s",
              syn_model_name(model[stick].info_sensor));
   ...
   gpm_report(GPM_PR_INFO, "   newABS:      %s",
              model[stick].info_new_abs ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   simpleCmd:   %s",
              model[stick].info_simple_cmd ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   geometry:    %d", model[stick].info_geometry);
   gpm_report(GPM_PR_INFO, "   extended:    %s",
              capabilities[stick].cap_ext ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   stick:       %s",
              capabilities[stick].cap_stick ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   sleep:       %s",
              capabilities[stick].cap_sleep ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   4 buttons:   %s",
              capabilities[stick].cap_four_button ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   multifinger: %s",
              capabilities[stick].cap_multi_finger ? "Yes" : "No");
   gpm_report(GPM_PR_INFO, "   palmdetect:  %s",
              capabilities[stick].cap_palm_detect ? "Yes" : "No");
...
static void syn_extract_extended_capabilities(int ext_cap_int,
                                              ext_cap_type * cap)
{
   ...
   if(cap->cap_ext) {
      ...
      /*
       * Wmode is not supported, but this should be turned off after reading
       * the config file.
       */
   }
}
      ...
      if(check_bits(mask, action_list[i].action_mask)) {
         switch (action_list[i].action) {
            ...
            case Up_Button_Action:
               scrolling_amount_left +=
                  ceil(scrolling_button_factor * scrolling_speed);
               status = GPM_B_UP;
               break;
            ...
            default:
               gpm_report(GPM_PR_WARN,
                          "Default Action: Action no. %X not defined",
...
static void tp_process_corner_taps(Gpm_Event * state, report_type * report)
{
   ...
   if(report->gesture &&
      ...
      last_corner_action =
         tp_process_action(corner_actions, edges_at_cornertap_time);
...
static void tp_process_finger_taps(Gpm_Event * state, report_type * report)
{
   ...
   if(report->gesture && last_corner_action == GPM_B_NOT_SET &&
      ...
      if((multi_finger_tap_enabled && (was_fingers == 0 || was_fingers == 1)) ||
         ...
         last_finger_action =
            tp_process_action(multi_finger_actions, ONE_FINGER);

      if(multi_finger_tap_enabled && was_fingers == 2)
         last_finger_action =
            tp_process_action(multi_finger_actions, TWO_FINGERS);

      if(multi_finger_tap_enabled && was_fingers == 3)
         last_finger_action =
            tp_process_action(multi_finger_actions, THREE_FINGERS);
...
static inline void
tp_process_repeating_action(Gpm_Event * state, int feature, int last_feature,
                            int *last_action,
...
static inline void
tp_process_repeating_actions(Gpm_Event * state, int features, int last_features,
                             int last_actions[],
                             touchpad_action_type * action_list)
{
   ...
   for(feature_no = 0; feature_no < 8; feature_no++) {
      ...
      tp_process_repeating_action(state, features & feature_mask,
                                  last_features & feature_mask,
                                  &last_actions[feature_no], action_list,
                                  feature_mask);
   }
}
...
static void tp_process_button_press(Gpm_Event * state, report_type * report)
{
   ...
   tp_process_repeating_action(state, report->left, last_report.left,
                               &last_normal_button_actions[0],
                               normal_button_actions, LEFT_BUTTON);

   tp_process_repeating_action(state, report->right, last_report.right,
                               &last_normal_button_actions[1],
                               normal_button_actions, RIGHT_BUTTON);

   tp_process_repeating_action(state, report->middle, last_report.middle,
                               &last_normal_button_actions[2],
                               normal_button_actions, MIDDLE_BUTTON);

   tp_process_repeating_action(state, report->fourth, last_report.fourth,
                               &last_normal_button_actions[3],
                               normal_button_actions, FOURTH_BUTTON);

   tp_process_repeating_action(state, report->up, last_report.up,
                               &last_normal_button_actions[4],
                               normal_button_actions, UP_BUTTON);

   tp_process_repeating_action(state, report->down, last_report.down,
                               &last_normal_button_actions[5],
                               normal_button_actions, DOWN_BUTTON);
}
...
{
   /*
    * STIG page 9: Values of w, vary from pad to pad. It is not precise when
    * pressure is small < 25. 4-7 finger of normal width 8-14 very wide finger
    * or palm 15 maximum reportable width 
    */
                                            ...
                                            Gpm_Event * state)
{
   ...
   if((report->pressure == 0 && (report->x != 0 || report->y != 0)) ||
       ...
       * cannot be processed further than here as the pressure is 0, which
       * would * break things, if the user uses it simultaneously with the
       * touchpad. 
       */
      /*
       * Allow some simultaneously usage: tap-hold on touchpad, with extended
       * movement. Do not do buttons, as they are not always correctly defined
       * yet. 
       */
      if(last_report.gesture) {
         report->gesture = 1;
         if(tap_gesture_enabled)
            tp_process_finger_taps(state, report);
         if(corner_taps_enabled)
       ...
       */
      /*
       * Stick pressed: The stick do only generates one packet, so double tap
       * is a problem; squeezing in a non-clicked state. It is probably not a
       * real problem, as it is hard not to move the stick a little between
       * pressing, thereby returning a non-clicked state between the packets.
       * Maybe press-lock mechanism is useful. Forget it, the styk supports it! 
       */
      if((data[0] & 0xFC) == 0x84 &&
         (data[1] & 0xC8) == 0x08 && (data[3] & 0xFC) == 0xC4) {
...
#ifdef DEBUG_STICK
         gpm_report(GPM_PR_DEBUG,
                    "StickData? %02x %02x %02x %02x %02x %02x :dx:%d dy:%d b:%d",
      ...
      if((data[0] & 0xFC) == 0x80 &&
         (data[1] & 0xFF) == 0x00 &&
         (data[3] & 0xFC) == 0xC0 && report->x < 4 && report->y < 4) {
         ...
         if(four_way_button_enabled) {
            ...
            if(four_way_button_is_mouse) {
               ...
               tmp_buttons |= ((report->y & 2) ? FOUR_RIGHT_BUTTON : 0);        /* RIGHT 
                                                                                 */
               tp_process_repeating_actions(state, tmp_buttons,
                                            last_4_way_buttons,
                                            &last_4_way_button_actions[0],
                                            four_button_actions);
            }
         }
...
static int tp_find_fingers(report_type * report, Gpm_Event * state)
{
...
#ifdef DEBUG_PALM
      gpm_report(GPM_PR_DEBUG, "\rTouchpad: palm detected. finger width: %d",
                 report->fingerwidth);
#endif
      /*
       * BUG should not return 1, as this drops packets. Return a repeated
       * report ? 
       */
      /*
   ...
   /*
    * Extra check for vertical multi fingers which my pad is very bad to
    * detect. ** Only check for extra fingers if no of fingers has not changed. 
    * Faking ** fingers may go wrong so sanity check needed.  This is not an
    * attempt to ** know the number of fingers all the time, as this is not
    * needed. 
    */
   if(fake_finger_layer_enabled) {
      ...
      if(report->fingers == 0) {
         ...
         multi_finger_xy = 0;
#endif
      } else if(report->fingers + fake_extra_finger == last_report.fingers) {

         multi_finger_pressure =
            sqr(report->pressure) - sqr(last_report.pressure);
         multi_finger_xy =
            (sqr(last_locs[mod4(packet_num - 1)].x - report->x) +
             sqr(last_locs[mod4(packet_num - 1)].y - report->y));

         /*
          * Check for a second finger. If the move is larger than tap range, *
          * then it is not a tap, and adding the finger does not change
          * reported * buttons, but it will still stop the moving if *
          * multi_finger_stop_enabled is on. These tests are complete base on
          * my * imagination and experience, so any better idea are welcome. 
          */
         if(report->fingers == 1 &&
            multi_finger_pressure > 4500
            && multi_finger_xy > 2 * sqr((double) tap_range)) {
            fake_extra_finger = 1;
            was_fake_pressure = report->pressure;

         } /* Check for third finger. */
         else if(last_report.pressure > 180
                 && (report->fingers + fake_extra_finger) == 2
                 && multi_finger_pressure > 4500
                 && multi_finger_xy > 2 * sqr((double) tap_range)) {
            fake_extra_finger = 2;
            was_fake_pressure = report->pressure;
         ...
         } else if(multi_finger_pressure < -5000
                   && multi_finger_xy > 2 * sqr((double) tap_range)) {
            /*
   ...
   if(multi_finger_stop_enabled) {
      ...
      if((report->fingers > 1 && report->fingers > last_report.fingers) ||
         ...
         /*
          * Updating the timer right after another added/removed finger, would
          * make the undo moving become redo, so dont.
          */
      ...
      if(multi_finger_stop_timer > 0) {
         ...
         if(multi_finger_stop_timer == multi_finger_stop_delay) {
            last_locs[mod4(packet_num - 2)].x +=
               last_state.dx * 2 / standard_speed_factor;
            last_locs[mod4(packet_num - 2)].y -=
               last_state.dy * 2 / standard_speed_factor;
         }
...
static void tp_find_gestures(report_type * report)
{
   ...
   if(report->fingers > 0) {
      ...
      /*
       * dragging and consecutive tap gestures is to end with finger up forget
       * fast that there was a tap if this is not a part of a tap.
       */
       ...
       */
      if((finger_on_pad_timer > (tap_lower_limit_packet)) &&    /* minimum
                                                                 * finger down
                                                                 * time */
         (finger_on_pad_timer < (tap_upper_limit_packet)) &&    /* maximum
                                                                 * finger down
                                                                 * time */
         ((distance((double) (stroke_x - last_report.x),        /* maximum
                                                                 * range for
                                                                 * finger to
                                                                 * drift while
                                                                 * down */
         ...
         if(time_to_forget_tap == 0)
            ...
            gesture_delay = multiple_tap_delay * 80 / 1000;     /* delay
                                                                 * gesture to
                                                                 * create
                                                                 * multiple
                                                                 * click */
      ...
      if(finger_on_pad_timer)
         gpm_report(GPM_PR_DEBUG, "A tap? %d < %d < %d && (%d)^2 + (%d)^2 < %d",
                    tap_lower_limit_packet, finger_on_pad_timer,
                    tap_upper_limit_packet, stroke_x - last_report.x,
                    stroke_y - last_report.y, tap_range * tap_range);
#endif
   ...
   report->gesture = ((time_to_forget_tap > 0) && (gesture_delay == 0))
      || drag_locked;
}
...
static void tp_process_report(Gpm_Event * state, report_type * report)
{
   ...
   if(report->fingers > 0) {
      ...
      if(tossing_enabled) {
         /*
          * this is the cue to stop tossing, if we are tossing so Force a touch 
          * free pause before a new tossing is allowed (prevent toss time) 
          */
      ...
      /*
       * Save the edge state, so we can detect a movement from non edge to
       * edge. 
       */
      if(!edges)
         was_non_edge = 1;
      ...
      /*
       * Need enough packets to perform smoothing on dy and dx * but it should
       * work with only 2, so why is 4 required? 
       */
      if(packet_num > 3) {
         ...
         edge_motion_on |= (edges && tap_hold_edge_motion_enabled
                            && last_report.gesture && was_non_edge);
         ...
         if(report->pressure > speed_up_pressure) {
            pressure_speed_factor *=
               1.0 +
               ((report->pressure - speed_up_pressure) * speed_pressure_factor);
         }
         ...
         if(edge_motion_on && (edges & TOP_EDGE))
            ...
            state->dy = (pressure_speed_factor *
                         (((last_locs[mod4(packet_num - 1)].y +
                          ...
                          ((loc.y + last_locs[mod4(packet_num - 1)].y) / 2)));
      ...
      if(tossing_enabled && was_fingers == 1 &&
         !was_tossing &&
         !is_scrolling &&
         (packet_num > min_toss_packets) && (packet_num < max_toss_packets)) {
...
#if DEBUG_TOSS
            gpm_report(GPM_PR_INFO, "tossx: %d  tossy: %d", toss_speed.x,
                       toss_speed.y);
#endif
         }
      }
    ...
    */
   if((scrolling_enabled && is_scrolling) || is_always_scrolling) {
   ...
   /*
    * check for (corner)buttons if we didn't just complete a toss or is
    * scrolling 
    */
   if(!is_tossing && !was_tossing && !is_scrolling) {
       ...
       */
      if(!report->gesture &&
         !report->left && !report->right && !report->middle
         && !report->fourth) {
   ...
   /*
    * Is there any amount of scrolling left? Should be checked after corner
    * actions. 
    */
   if(scrolling_amount_left != 0) {
      tp_handle_scrolling(state);
...
void tp_read_config_file(char *config_filename)
{
   ...
   if(status < 0) {
      gpm_report(GPM_PR_WARN, "Too long path for configure file: %s",
                 config_filename);
      return;
   }
                  ...
                  switch (param_data[param].p_type) {
                     case Integer_Param:
                        ...
                        if(end_ptr != token)
                           *(param_data[param].addr.int_p) = tmp_read_int_param;
                           ...
                           gpm_report(GPM_PR_WARN,
                                      "Integer value (%s) for parameter %s is invalid",
                     ...
                     case Float_Param:
                        ...
                        if(end_ptr != token)
                           *(param_data[param].addr.float_p) =
                              tmp_read_float_param;
                        else
                           gpm_report(GPM_PR_WARN,
                                      "Float value (%s) for parameter %s is invalid",
                                      token, param_data[param].name);
#if DEBUG_PARAMS
                     ...
                     case Flag_Param:
                        if(strchr("YyTt1", token[0])) {
                           ...
                           *(param_data[param].addr.flag_p) = 0;
                           ...
                           gpm_report(GPM_PR_WARN,
                                      "Flag value (%s) for parameter %s is invalid",
                                      token, param_data[param].name);
                        }
#if DEBUG_PARAMS
                     ...
                     case Action_Param:
                        ...
                        if(end_ptr != token)
                           ...
                           gpm_report(GPM_PR_WARN,
                                      "Action value (%s) for parameter %s is invalid",
                                      token, param_data[param].name);
#if DEBUG_PARAMS
                        gpm_report(GPM_PR_INFO, "Param %s set to %d",
                                   param_data[param].name,
...
*/
static void syn_process_config(info_type ident, model_id_type model)
{
    ...
    * done after reading the configure file so they may be turned off on
    * purpose and can'nt be turned on if not supported. 
    */
...
static void tp_serial_send_cmd(int fd, unsigned char *cmd)
{
...
static void syn_serial_set_mode(int fd, unsigned char mode)
{
...
static void syn_serial_read_ident(int fd, info_type * info)
{
   ...
   if(bytes[0] == 'S' && bytes[0] == 'T') {
      /*
       * reformat the data The out commented is wrong according to STIG page
       * 57.
       */
      /*
      ...
      gpm_report(GPM_PR_ERR,
                 "PS/2 serial device doesn't appear to be a synaptics touchpad\n");
...
#define PS2_MOUSE_IDCODE  0x00  /* Identification code (meaning mouse) sent
                                 * after a PS2_READY */

/* Additional synaptic commands*/
#define PS2_SYN_CMD       0xE8  /* Four of these each with an following byte
                                 * encodes a command */
#define PS2_SYN_INERT     0xE6  /* This ps2 command is ignored by synaptics */
#define PS2_SYN_SET_MODE1 0x0A  /* Set the mode byte 1 instead of sample rate
                                 * (used after a sample rate cmd) */
#define PS2_SYN_SET_MODE2 0x14  /* Set the mode byte 2 instead of sample rate
                                 * (used after a sample rate cmd). All other
                                 * sample rate gives undefined behavior (used
                                 * to address 4 byte mode) */
#define PS2_SYN_SET_STICK 0x28  /* Send byte to stick */
#define PS2_SYN_STATUS_OK 0x47  /* Special synaptics Status report is
                                 * recognized */
...
static byte tp_ps2_putbyte(int fd, byte b)
{
   byte ack;
 ...
 * whether it is the touchpad or stick device that is meant.  */
static void syn_ps2_putbyte(int fd, int stick, byte b)
{
   byte ack;

#ifdef DEBUG_PUTBYTE
   gpm_report(GPM_PR_DEBUG, "Send byte %X to %s", b,
              (stick ? "Stick" : "Touchpad"));
#endif
...
#ifdef DEBUG_CMD
   gpm_report(GPM_PR_DEBUG, "Send Command %X to %s", cmd,
              (stick ? "Stick" : "Touchpad"));
#endif
 ...
 */
static void syn_ps2_set_mode1(int fd, int stick, byte cmd)
{
 ...
 */
static void syn_ps2_set_mode2(int fd, int stick, byte cmd)
{
   syn_ps2_send_cmd(fd, stick, cmd);
...
*/
static void syn_ps2_status_rqst(int fd, int stick, byte cmd, byte * bytes)
{
   gpm_report(GPM_PR_INFO, "Status request for %s, %X",
              (stick ? "stick" : "touchpad"), cmd);
   ...
   gpm_report(GPM_PR_INFO, "Status request %X %X %X", bytes[0], bytes[1],
              bytes[2]);
}
...
#   ifdef DEBUG
   gpm_report(GPM_PR_INFO, "Synaptic PS/2 %s modes: %02X",
              (stick ? "stick" : "touchpad"), bytes[2]);
#   endif
}
...
static void syn_ps2_read_ident(int fd, int stick, info_type * info)
{
   byte bytes[3];
   ...
   if(bytes[1] != PS2_SYN_STATUS_OK) {
      gpm_report(GPM_PR_ERR,
                 "PS/2 device doesn't appear to have synaptics %s identification\n",
...
static void syn_ps2_read_model_id(int fd, int stick, model_id_type * model)
{
   ...
   model_int = ((bytes[0] << 16) | (bytes[1] << 8) | (bytes[2]));
   syn_extract_model_id_info(model_int, model);
}
...
static void syn_ps2_read_cap(int fd, int stick, ext_cap_type * cap)
{
   ...
   if(bytes[1] != PS2_SYN_STATUS_OK) {
      gpm_report(GPM_PR_ERR,
                 "PS/2 device doesn't appear to have synaptics %s capabilities\n",
                 (stick ? "stick" : "touchpad"));
      ext_cap_int = 0;
   } else {
...
static void tp_ps2_disable_data(int fd)
{
   ...
   if(status != PS2_ACK)
      gpm_report(GPM_PR_ERR, "Invalid ACK to disable stream command, got %02X",
                 status);
}
...
static void syn_ps2_send_reset(int fd, int stick)
{
   ...
   /*
    * Sometimes the touchpad sends additional ready,idcode before ack the reset 
    * command. I dont know why! 
    */
   while(status == PS2_READY) {
      gpm_report(GPM_PR_INFO,
                 "PS/2 device sending additional READY, ID CODE.\n");
   ...
   if(status != PS2_ACK || id_code != PS2_MOUSE_IDCODE) {
      gpm_report(GPM_PR_ERR,
                 "Sending reset command to PS/2 Device failed: No ACK, got %02X.\n",
                 status);
   }
...
static void syn_serial_translate_data(unsigned char *data, report_type * report)
{
...
static void syn_ps2_translate_error(unsigned char *data, report_type * report)
{
   gpm_report(GPM_PR_WARN,
              "Unrecognized Synaptic PS/2 Touchpad packet: %02X %02X %02X %02X %02X %02X",
              data[0], data[1], data[2], data[3], data[4], data[5]);

   if(reset_on_error_enabled) {
      /*
       * Hack to get the fd: which_mouse is the current mouse, and as the
       * synaptic code is called, it is the current mouse. 
       */
      syn_ps2_reset(which_mouse->fd);
      syn_ps2_absolute_mode(which_mouse->fd);
   }
...
/* Translate the reported data into a record for processing */
static void syn_ps2_translate_data(unsigned char *data, report_type * report)
{
   ...
   if(((data[0] & 0xc8) == 0x80) &&     /* Check static in byte 0 */
      ...
      report->x = (((data[1] & 0x0F) << 8) |
                   ((data[3] & 0x10) << 8) | ((data[4])));
      report->y = (((data[1] & 0xF0) << 4) |
                   ((data[3] & 0x20) << 7) | ((data[5])));
      ...
      report->up = 0;
      report->down = 0;
      report->x = (((data[1] & 0x1F) << 8) | ((data[2])));
      report->y = (((data[4] & 0x1f) << 8) | ((data[5])));
      report->pressure = (((data[0] & 0x30) << 2) | ((data[3] & 0x3f)));
      report->gesture = check_bits(data[0], 0x04);
...
*/
void syn_process_serial_data(Gpm_Event * state, unsigned char *data)
{
   /*
   ...
   if(tp_find_fingers(&cur_report, state))
      return;
   if(wmode_enabled) {
...
void syn_serial_reset(int fd)
{
   ...
   syn_serial_set_mode(fd, (RELATIVE_MODE |
                            HIGH_REPORT_RATE |
                            USE_9600_BAUD | NORMAL_REPORT | REPORT_W_OFF));
...
** Process the touchpad 6 byte report.
*/
void syn_process_ps2_data(Gpm_Event * state, unsigned char *data)
{
   /*
    * gpm_report(GPM_PR_DEBUG,"Data %02x %02x %02x %02x %02x
    * %02x",data[0],data[1],data[2],data[3],data[4],data[5]); 
    */
   ...
   if(wmode_enabled) {
      ...
      if(syn_ps2_process_extended_packets(data, &cur_report, state))
         return;
      syn_process_wmode_report(&cur_report);
      if(tp_find_fingers(&cur_report, state))
         return;
