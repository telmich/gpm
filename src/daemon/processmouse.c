...
int processMouse(int fd, Gpm_Event *event, Gpm_Type *type, int kd_mode)
{
   ...
   fd_set                  fdSet;
   oldT = event->type;
   (*event).dev=(which_mouse->opt_dev);/*Asign the device to the event*/
   ...
   if(eventFlag) {
      ...
      do { /* cluster loop */
         if(((data=getMouseData(fd, (which_mouse->m_type), kd_mode)) == NULL)
            ...
            if (!i) {
               printf("return 1-0 data in event:%s\n",event->dev);
               return 0;
            } else {
               break;
            }
         }
	 nEvent.dev=(which_mouse->opt_dev);	
...
/*....................................... we're a repeater, aren't we? */
   ...
   if (kd_mode!=KD_TEXT) {
      ...
      printf("return-0 2 data in event:%s\n",event->dev);
   ...
   if (!event->dx && !event->dy
      ...
      do { /* so to break */
         ...
         if (time(NULL)<=awaketime){
             printf("return 3-0 data in event:%s\n",event->dev); 
             return 0;
         }
 ...
 */
   ...
   gpm_report(GPM_PR_DEBUG,"device: %s dx: %3i dy: %3i x: %3i y: %3i butt: %i vc: %i clicks: %i",
                                       event->dev,
				       event->dx,event->dy,
                                       event->x,event->y,
   ...
   if (opt_special && event->type & GPM_DOWN){
      printf("retunr 4-? data in event:%s\n",event->dev); 
      return processSpecial(event);
   }

   printf("return 5-1 data in event:%s\n",event->dev);
   return 1;
}
