...
int Gpm_Open(Gpm_Connect * conn, int flag)
{
   ...
   if(!consolename) {
      ...
      if(!consolename) {
         gpm_report(GPM_PR_ERR,
                    "unable to open gpm console, check your /dev filesystem!\n");
         goto err;
      }
   }
   ...
   if(putdata(gpm_fd, conn) != -1) {
      /*
       * itz Wed Dec 16 23:22:16 PST 1998 use sigaction, the old code caused a
       * signal loop under XEmacs 
       */
      struct sigaction sa;
...
int Gpm_GetEvent(Gpm_Event * event)
{
   ...
   if((count = read(gpm_fd, &magic, sizeof(int))) != sizeof(int)) {
      ...
      gpm_report(GPM_PR_INFO, "Read too few bytes (%i) at %s:%d", count,
                 __FILE__, __LINE__);
      return -1;
   }
#endif
...
#define DELAY_MS 100
      ...
      while(1) {
         do {
            ...
            gpm_timeout.tv_sec = SELECT_TIME;
            flag =
               select(fd + 1, &selSet, (fd_set *) NULL, (fd_set *) NULL,
                      &gpm_timeout);
         }
         while(!flag);
    ...
    */
      ...
      return fgetc(f);          /* no mouse available */
}
...
#define DIF_TIME(t1,t2) ((t2.tv_sec -t1.tv_sec) *1000+ \
   ...
   if(c == 3) {
      ...
      ePtr->type = GPM_DOWN;
      GET_TIME(tv2);
      if(tv1.tv_sec && (DIF_TIME(tv1, tv2) < 250)) {    /* 250ms for double
                                                         * click */
         clicks++;
         clicks %= 3;
      } else
