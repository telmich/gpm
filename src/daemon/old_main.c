...
int old_main()
{
   ...
   for(i = 1; i <= 1 + opt_double; i++) {
       ...
       */
      mouse_argv[i] =
         build_argv((which_mouse->opt_type), (which_mouse->opt_options),
                    &mouse_argc[i], ',');
...
/*....................................... catch interesting signals */
   ...
   signal(SIGWINCH, gpm_killed);        /* winch can be sent if console is
                                         * resized */
...
/*--------------------------------------- main loop begins here */
   ...
   while(1) {
      ...
      if(eventFlag) {           /* an event left over by clustering */
         ...
         while((pending =
                select(maxfd + 1, &selSet, NULL_SET, NULL_SET,
                       &timeout)) == 0) {
            selSet = readySet;
            resetTimeout();
         }                      /* go on */
      ...
      if(pending < 0) {
         if(errno == EBADF)
            gpm_report(GPM_PR_OOPS, GPM_MESS_SELECT_PROB);
      ...
      for(i = 1; i <= 1 + opt_double; i++) {
         ...
         if(FD_ISSET(which_mouse->fd, &selSet)) {
            ...
            if(processMouse(which_mouse->fd, &event, kd_mode)) {
               ...
               /*
                * pass it to the client, if any or to the default handler, if
                * any or to the selection handler 
                */
               ret = 0;
               if(cinfo[event.vc])
                  ret = do_client(cinfo[event.vc], &event);
               if(!ret && cinfo[0])
                  ret = do_client(cinfo[0], &event);
               if(!ret)
                  do_selection(&event);
               /*
                * Same idea as above, just here for documentation (cinfo[event.vc] 
                * && do_client(cinfo[event.vc], &event)) || (cinfo[0] &&
                * do_client(cinfo[0], &event)) || do_selection(&event); 
                */
            }
         }
      }
      ...
      for(i = 0; pending && (i < MAX_NR_CONSOLES); i++) {
         ...
         for(ci = cinfo[i]; pending && ci; ci = ci->next) {
            if(FD_ISSET(ci->fd, &selSet)) {
                ...
                */
               /*
                * this code is clearly incorrect; the next highest descriptor
                * after the one we're closing is not necessarily being used.
                * Fortunately, it doesn't hurt simply to leave this out. 
                */
