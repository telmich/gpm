...
#include "daemon.h"             /* FIXME: do not use that..rewrite mouse-test,
                                 * anyway */
#include "drivers.h"
...
int mousereopen(int oldfd, char *name, Gpm_Type * type)
{
   ...
   fd = open(name, O_RDWR);
   if(fd < 0)
      gpm_report(GPM_PR_OOPS, name);
   (*mt_I_serial) (fd, type->flags, type, 1, &type->name);      /* ms
                                                                 * initialization 
                                                                 */
   return fd;
}
...
int eventlist(int fd, unsigned char *buff, int buflen, int test, int readstep)
{
   ...
   while(1) {
      selSet = readySet;
      if((pending = select(fd + 1, &selSet, NULL, NULL, &to)) < 0)
         continue;
      if(pending == 0 && !active) {
         active++;
         to.tv_sec = 10;
...
/*====================================== First of all, detect the device */
   ...
   while(devcount > 1) {
       ...
       * BUG 
       *//*
       * Logitech initialization is not performed 
       */
      ...
      for(nextdev = &devlist; *nextdev; nextdev = &((*nextdev)->next)) {
         ...
         (*mt_I_serial) ((*nextdev)->fd, (mice + 1)->flags, mice + 1, 1, &(mice + 1)->name);    /* try 
                                                                                                 * ms 
                                                                                                 * mode 
                                                                                                 */
      }
